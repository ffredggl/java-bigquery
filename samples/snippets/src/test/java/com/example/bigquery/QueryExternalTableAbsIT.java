...
public class QueryExternalTableAbsIT {
  ...
  @Test
  public void testQueryExternalTableAbs() throws InterruptedException {
    String query =
        String.format(
            "SELECT * FROM %s.%s.%s WHERE name LIKE 'W%%'",
            OMNI_PROJECT_ID, OMNI_DATASET_NAME, OMNI_EXTERNAL_TABLE_NAME);
    QueryExternalTableAbs.queryExternalTableAbs(query);
